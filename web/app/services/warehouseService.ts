...
export async function submitWarehouse(
  ...
  // @ts-ignore
  return fetchApi<Repository>(window.API_URL + '/api/Warehouse/SubmitWarehouse', {
    method: 'POST',
...
export async function getWarehouse(page: number, pageSize: number): Promise<ApiResponse<WarehouseListResponse>> {
  // @ts-ignore
  return fetchApi<WarehouseListResponse>(window.API_URL + '/api/Warehouse/WarehouseList?page=' + page + '&pageSize=' + pageSize, {
    method: 'GET',
    // 添加缓存控制使其适用于服务器组件
    cache: 'no-store'
  });
...
export async function documentCatalog(organizationName:string,name:string):Promise<any>{
  // @ts-ignore
  return fetchApi<any>(window.API_URL + '/api/DocumentCatalog/DocumentCatalogs?organizationName=' + organizationName + '&name=' + name, {
    method: 'GET',
    // 添加缓存控制使其适用于服务器组件
    cache: 'no-store'
  });
...
export async function documentById(owner:string, name:string, path:string):Promise<any>{
  // @ts-ignore
  return fetchApi<any>(window.API_URL + '/api/DocumentCatalog/DocumentById?owner=' + owner + '&name=' + name + '&path=' + path, {
    method: 'GET',
    // 添加缓存控制使其适用于服务器组件
    cache: 'no-store'
  });
...
export async function getWarehouseOverview(owner:string, name:string){
  // @ts-ignore
  return fetchApi<any>(window.API_URL + '/api/Warehouse/WarehouseOverview?owner=' + owner + '&name=' + name, {
    method: 'GET',
    // 添加缓存控制使其适用于服务器组件
    cache: 'no-store'
  });
...
export async function getLastWarehouse(address:string){
  // @ts-ignore
  return fetchApi<any>(window.API_URL + '/api/Warehouse/LastWarehouse?address=' + address, {
    method: 'GET',
    // 添加缓存控制使其适用于服务器组件
    cache: 'no-store'
  });
